    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      <div>
        ...
        <ul className="mt-3 space-y-1 text-xs text-emerald-400">
          <li className="flex gap-1"><CheckCircle className="h-3 w-3 mt-0.5" /> &ldquo;COFFEE BLENDED WITH CHICORY&rdquo; on front panel</li>
      ...
      <div className="mb-4">
        ...
        <ul className="space-y-2">
          {finding.evidence.map((e: string, i: number) => (
            <li key={i} className="text-sm text-gray-300">&ldquo;{e}&rdquo;</li>
          ))}
        </ul>
      </div>
    )}
...
const [selectedTimeline, setSelectedTimeline] = useState<{ timeframe: string; actions: TimelineAction[] } | null>(null);
  ...
  const runCompliance = useCallback(async () => {
    ...
    try {
      ...
      const findings = byAmendment.map((a: Finding) => ({
      ...
      const detailedAmendments = byAmendment.map((a: Finding) => ({
        title: a.amendment_title,
        date: a.last_date || 'Unknown',
  ...
  }, [API_BASE, companyId, amendments, selectedTimeline]);
                      ...
                      <div className="mt-4">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          ...
                          <div>
                            ...
                            <Image 
                              src={action.requiredLabel || '/required-label.png'} 
  ...
  const AmendmentCard = ({ amendment }: { amendment: Amendment }) => (
    <div className="bg-gray-900 border border-gray-800 rounded-xl p-5 hover:border-gray-700 transition-colors">
      ...
      {amendment.product_impacts?.length && (
        <div className="mb-4">
          ...
          <div className="space-y-3">
            {amendment.product_impacts.map((impact: ProductImpact, i: number) => (
              ...
              {analysis.findings?.length ? (
                <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
                  ...
                  <div className="grid grid-cols-1 gap-4">
                    {analysis.findings.map((f, i: number) => (
